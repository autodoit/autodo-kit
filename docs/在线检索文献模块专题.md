...
这份专题文档的目标不是继续堆叠“中文一套、英文一套”的分叉实现，而是把模块重写成一套统一、可扩展、可审计的结构，明确回答三件事：
...
2. 路由层只负责识别请求画像、注入策略和选择编排链路。
3. 编排层负责来源发现、来源筛选、输入规范化、批量组织、失败重试与结果汇总。
4. 执行层负责真实的单篇 metadata、单篇 download、单篇 structured extract。
5. A040、Bib 批处理等上层事务只能通过统一路由入口使用本模块，不能直接复制内部能力。

## 2. 三层结构

### 2.1 结构定义

本模块按三层结构组织，再叠加一个上位的请求画像层作为输入约束：
...
其中，请求画像层不是实现层，而是决定路由优先级和来源族选择的约束层。

### 2.2 详细结构图

```mermaid
flowchart TD
    U[上层调用方<br/>A040 / Bib 批处理 / 调试脚本] --> P[统一 payload]
    P --> RQ[请求画像层<br/>中文 / 英文 / 中英文不限]

    RQ --> RT[路由层<br/>online_retrieval_router.py]
    RT --> OP[编排层<br/>resolver / service / policy / retry]
    OP --> EX[执行层<br/>单篇 metadata / download / structured extract]

    OP --> ND[学校数据库导航门户<br/>来源发现 / 来源选择 / access probe]
    OP --> BT[批量编排与断点续跑]
    OP --> RS[失败项重试与回填]

    EX --> CN[内容门户执行器<br/>CNKI / SPIS]
    EX --> OA[开放平台执行器<br/>OpenAlex / Crossref / Europe PMC / arXiv]
    ...
    ND -. 只做来源筛选与辅助 .-> OP
    BT -. 组织 entries / records / seed_items .-> EX
    RS -. retry only .-> EX
```
...
1. 请求画像先于动作，不是动作本身。
2. 路由层只决定走哪条链，不直接做单篇业务。
3. 编排层负责把批量输入整理成执行层可消费的单篇任务。
4. 执行层才是真正的单篇能力真相源。
...
如果把两者混在一起，routing constraint 和 capability 就会变成同一维度，后续会越来越难维护。
...
能力矩阵建议使用“来源族 × 动作能力”，并为每个格子补充状态属性。

### 6.1 矩阵总览

| 来源族 | metadata search | single download | batch download | single structured extract | batch structured extract | access probe / source selection | retry |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 内容门户 | 实现中，执行层主能力 | 实现中，执行层主能力 | 部分实现，优先由编排层调度 | 实现中，执行层主能力 | 部分实现，优先由编排层调度 | 可辅助，不是主职责 | 可参与失败回退 |
| 开放平台 | 实现中，执行层主能力 | 实现中，执行层主能力 | 部分实现，优先由编排层调度 | 实现中，执行层主能力 | 部分实现，优先由编排层调度 | 可辅助，不是主职责 | 可参与失败回退 |
| 学校数据库导航门户 | 不承担主检索 | 不承担主下载 | 不承担主下载 | 不承担主提取 | 不承担主提取 | 主职责，编排层核心 | 主职责，编排层核心 |
...
4. 如果某项能力仍需人工接管，应在矩阵单元格直接标注，不要隐藏在实现细节里。
...
flowchart LR
    subgraph Entry[入口层]
        R1[online_retrieval_router.py]
    end

    subgraph Route[路由层]
        P1[retrieval_policy.py]
        S1[online_retrieval_service.py]
        L1[online_retrieval_resolver.py]
    end

    subgraph Orchestration[编排层]
        F1[school_foreign_database_portal.py]
        F2[en_chaoxing_portal_retry.py]
        F3[来源发现 / 来源选择 / retry / 批量组织]
    end
...
这张图表达的是“入口、路由、编排、执行”之间的职责分离：

1. 入口文件只负责接住统一 payload。
2. 路由层决定策略，不直接执行单篇任务。
3. 编排层决定来源与重试，不直接承担单篇核心能力。
4. 执行层才真正做文献动作。
