...
1. `workspace/config` 的键名契约（含嵌套）默认保持既有结构，不做无差别中英重写。
2. `content.db` 中 `uid_` 前缀字段、`cite_key` 字段、`bib_*` 字段为英文例外白名单。
3. 历史列名 `题录键`、`来源题录键`、`阅读来源题录键` 在初始化阶段会兼容回迁到 `cite_key`/`source_cite_key`。
...
A060-A160 这段链路在不同文档中会同时出现“节点编号名”“官方 affair 名”“宿主补位智能体名”三套称呼。当前统一规则如下：

1. 以宿主注册表映射到的官方 affair 为默认执行入口。
2. 以 autodo-kit 中 `autodokit.affairs.<事务名>.affair.execute` 为 API 真入口。
3. 以宿主编号事务智能体为补位与解释层，不把它们当成 autodo-kit 默认执行者。

特别说明：

1. 当前主链 A060 官方入口的 technical uid 是 `ar_A060_综述文献候选视图构建`，模块名仍保留 `候选文献视图构建`，不是 legacy 的 `综述预处理`。
2. 当前主链 A070 官方入口的 technical uid 是 `ar_A070_综述文献研读`，同样映射到 `候选文献视图构建` 模块，但执行模式应切到 `review_reading`。
3. 当前主链 A075 官方入口的 technical uid 是 `ar_A075_普通文献候选视图构建`，模块仍映射到 `非综述候选种子生成`。
4. A080 当前宿主职责是“普通文献泛读”，其 official affair_uid 为 `ar_A080_普通文献泛读`，负责预处理与解析落盘，再把条目推进到 A095。
5. A095 当前宿主职责是“普通文献研读候选视图构建”，其 official affair_uid 为 `ar_A095_普通文献研读候选视图构建`，模块与节点职责已对齐。
6. A100 当前宿主职责通常表述为“文献批判性研读”，其 official affair_uid 为 `ar_A100_文献批判性研读`，模块名仍保留 `文献研读与正式知识回写`。
7. A140 当前宿主职责通常表述为“创新点凝练”，其 official affair_uid 为 `ar_A140_创新点凝练`，模块名仍保留 `创新点池构建`。

完整矩阵与柔性简并视图见 [A060-A160事务对齐说明](A060-A160事务对齐说明.md)。

## 0. A080-A100 普通文献流程状态契约

当前普通文献阅读主链的正式节点顺序是：A080 -> A095 -> A100。

状态真相源统一为 `content.db` 的 `文献流程状态`：

1. A075 或人工候选把普通文献写入 `流程轨道='普通主链'`、`当前阶段='待预处理'`、`当前状态='待处理'`。
2. A080/A095/A100 统一按 `流程轨道`、`节点编码`、`当前阶段`、`当前状态`、`是否可执行` 筛选和推进。
...
| --- | --- | --- | --- | --- |
| 待预处理 | `当前阶段='待预处理'`、`当前状态='待处理'` | A080 | A075、人工候选、回流节点 | 当前普通链候选由 A075 或人工候选写入，再进入 A080。 |
| 待泛读候选构建 | `当前阶段='普通文献研读候选视图构建'`、`当前状态='待处理'` | A095 | A080 | A080 预处理完成后把可继续阅读的条目推进到 A095。 |
| 预处理完成 | `当前阶段='待精解析'`、`来源阶段='普通文献研读候选视图构建'` | A100 | A095 | A095 负责粗读、轻量分析与批次汇总，并把适合深读的条目推进到 A100。 |
| 精解析中 | `当前阶段='精解析'`、`当前状态='处理中'` | - | A100 | 当前轮已被 A100 接管。 |
| 批判性研读 | `当前阶段='批判性研读'` | A100 | A100 | A100 在同一外部节点内完成标准笔记与知识回写。 |
| 普通主链完成 | `当前状态='已完成'`、`是否当前有效=1` | 下游节点 | A100 | 已完成至少一轮批判性研读并在 A100 内收口。 |
| 条目来源 | `来源类型`、`来源阶段`、`推荐原因` | A080/A095/A100 | 上游节点 | 记录人工、自动、回流或恢复来源。 |
| 阅读提示 | `阅读目标`、`人工提示` | A095/A100 | A075/A080/人工修订 | 逐文献阅读目标与人工提示。 |

### 0.2 当前实现流程

1. A080 读取 `当前阶段='待预处理' AND 当前状态='待处理' AND 是否可执行=1`，成功后写入解析资产，并把 `下一阶段` 或 `当前阶段` 推进到 `普通文献研读候选视图构建`。
2. A095 接管 `普通文献研读候选视图构建` 条目时先写 `当前状态='处理中'`，完成后写 `rough_read_done=1`、`analysis_batch_synced=1` 与 `pending_deep_read=1`，并按条件把下一阶段置为 `待精解析`。
3. A100 接管 `待精解析` 条目时先写 `当前状态='处理中'`，成功后登记 `文献解析资产`，并推进到 `批判性研读`。
4. A100 在同一外部节点内完成标准笔记与知识回写，并把流程状态收束为 `已完成`。
5. A095 与 A100 发现的新候选统一回写为 `流程轨道='普通主链'` 的新状态行，由 `当前阶段` 与 `是否可执行` 决定是否立即进入下游。
6. 已有当前有效流程状态的条目不重复新建状态行；确需改道时更新原状态的 `下一阶段`、`推荐原因` 和 `人工提示`。

### 0.3 human_seed_contract（A080/A095/A100 共用）

为什么 A080/A095/A100 都允许写 `seed_items`：

1. 统一契约：三个节点读取同一配置结构，便于同一批 seed 在不同轮次复用，不需要按节点维护三套格式。
2. 人工纠偏：真实运行中人工干预常发生在中途；允许在 A095/A100 继续补 seed，可避免“必须回到 A080 才能加单”的流程阻塞。
3. 职责不变：虽然都可写，但节点仍按状态机消费；A080 只做预处理落位，A095 只做粗读与批次汇总，A100 只做深读，不会越权。
...
- 普通文献阅读主链 A080-A100 统一以 `content.db` 的 `文献流程状态` 为正式真相源；候选入口通过 `流程轨道='普通主链'`、`当前阶段`、`当前状态` 表达。
...
- `invoke_aliyun_llm(...)`：按路由计划执行主模型调用，失败时按回退链重试，并返回统一 `attempts` 审计结构；当前 AOE 的 EA 自动审计也复用该入口。
...
4. `backfill_content_relationships(db_path)` 现在定位为历史资产 repair/upsert 工具；A020/A045/A100 等事务应直接写事实关系表。
...
- `write-log`：把一段文本写入 AOK 日志数据库（`workspace/database/logs/log.db`），可通过 `--message` 传入字符串，或 `--message-file` 指定文件，或从标准输入读取。
  - 典型示例：
    - `python tools/aok_tool/aok.py write-log --workspace workspace --message "已完成 A080，准备启动 A095"`
    - `python tools/aok_tool/aok.py write-log --workspace workspace --message-file ./notes/summary.txt`

- `list-queue`：读取工作区内容数据库（`workspace/database/content/content.db`）并按阶段列出候选队列简要视图（JSON 输出）。支持 `--stage` 指定阶段（如 `A060`、`A080`、`A095`），`--limit` 控制返回数量。
  - 典型示例：
...
- `--stage`：阅读队列阶段（`A060`/`A080`/`A095`/`A100` 等）。
