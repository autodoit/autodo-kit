...
1. 当前主链 A060 官方入口是 `候选文献视图构建`，不是 legacy 的 `综述预处理`。
2. A080 当前宿主职责是“非综述文献预处理”，其官方 affair 模块名仍保留 `非综述候选视图构建`。
3. A100 当前宿主职责通常表述为“文献精解析资产化”，其官方 affair 模块名仍保留 `文献研读与正式知识回写`。
4. A160 当前宿主职责通常表述为“报告收敛与交付”，其官方 affair 名仍为 `成果归档发布`。
...
| --- | --- | --- | --- | --- |
| 待预处理 | `当前阶段='待预处理'`、`当前状态='待处理'` | A080 | A060、人工候选、回流节点 | A060 已吸收原 A075 导种职责，普通链候选统一从这里进入 A080。 |
| 预处理完成 | `当前阶段='待精解析'`、`来源阶段='待预处理'` | A100 | A080 | A080 已内联吸收原 A090/A095 的粗读与批次汇总职责。 |
| 精解析中 | `当前阶段='精解析'`、`当前状态='处理中'` | - | A100 | 当前轮已被 A100 接管。 |
| 批判性研读 | `当前阶段='批判性研读'` | A100 | A100 | A100 已内联吸收原 A105 的标准笔记与知识回写职责。 |
