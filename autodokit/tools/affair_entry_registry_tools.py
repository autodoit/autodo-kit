...
MAINLINE_AFFAIR_ENTRY_MAP: dict[str, dict[str, Any]] = {
    ...
    "A050": {"node_name": "文献下载与主附件入库", "affair_uid": "ar_A045_文献下载与主附件入库", "module": "autodokit.affairs.文献下载与主附件入库.affair", "callable": "execute", "implemented": True},
    "A060": {
        "node_name": "预处理优先级生成",
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A060 对 content.db 文献主表全库生成 A050_REVIEW/A050_NON_REVIEW 当前优先级，并回写主文献表既有预处理摘要列（execution_mode=priority_only）。",
    },
    "A070": {
        "node_name": "统一文献预处理执行",
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A070 按主文献表当前预处理摘要列派生的 queued 队列持续消费，并以 priority ASC 执行具体预处理（execution_mode=full_preprocess）。",
    },
    "A110": {
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A110 重新作为综述链候选视图构建入口，仅负责生成综述候选池、阅读池、批次与 A130 当前态输入。",
    },
    "A130": {
        "node_name": "综述文献研读",
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A130 承接 A110 输出，当前外部节点内串行执行 A120 参考文献预处理与 A130 综述综合研读。",
    },
    "A140": {
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A140 重新作为普通链候选视图构建入口，负责消费 A130 导出件与人工种子并正式写入 A150 队列。",
    },
    "A150": {
        "node_name": "普通文献泛读",
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A150 当前负责普通文献预处理与解析落盘，并把可继续阅读的条目正式推进到 A160。",
    },
    "A160": {
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A160 承接 A150 输出，执行普通文献粗读、轻量分析与批次汇总，并把可深读条目推进到 A170。",
    },
    "A170": {
        "node_name": "文献批判性研读",
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A170 物理吸收 A105，当前主链显示名统一为文献批判性研读。",
    },
    "A180": {
        "node_name": "研究脉络梳理",
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A180 物理吸收 A120/A130，当前主链显示名统一为研究脉络梳理。",
    },
    "A190": {
        "node_name": "创新点凝练",
        ...
        "callable": "execute",
        "implemented": True,
        "notes": "A190 物理吸收 A150/A160，当前主链显示名统一为创新点凝练。",
    },
}
