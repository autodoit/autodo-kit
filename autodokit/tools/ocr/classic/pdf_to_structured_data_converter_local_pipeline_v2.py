...
- 当常规文本抽取为空时，使用 RapidOCR 对页面渲染图做 OCR 兜底；
- 参考文献抽取使用规则方法；
...
def _extract_text_fulltext_local(pdf_path: Path) -> tuple[str, str | None, Dict[str, Any]]:
    ...
    2) pypdf：逐页抽取文本，适合作为轻量兜底。
    3) pdfminer.six：仅全文。
    4) RapidOCR：前述文本抽取都没有有效结果时，对页面渲染图做 OCR 兜底。
    ...
    try:
        ...
        if text:
            return text, None, {"backend": "pypdf"}
        pypdf_err = "pypdf 返回空文本"
    except Exception as exc:
        pypdf_err = str(exc)
    ...
    try:
        ...
        if text.strip():
            return text, None, {"backend": "pdfminer"}
        pdfminer_err = "pdfminer 返回空文本"
    except Exception as exc:
        pdfminer_err = str(exc)

    # 最后尝试 RapidOCR 兜底，适合扫描版或纯图片 PDF。
    try:
        ocr_text, ocr_status, ocr_meta = extract_text_with_rapidocr(pdf_path)
        if ocr_text.strip():
            return ocr_text, None, {"backend": "rapidocr", "ocr": ocr_meta}
        ocr_err = ocr_status.disabled_reason or ocr_meta.get("ocr_error") or "RapidOCR 未识别到文本"
    except Exception as exc:
        ocr_err = str(exc)

    return "", (
        f"PyMuPDF 失败：{pymupdf_err}；"
        f"pypdf 失败：{pypdf_err}；"
        f"pdfminer 失败：{pdfminer_err}；"
        f"RapidOCR 失败：{ocr_err}"
    ), {"backend": "none", "ocr_attempted": True, "ocr_error": ocr_err}
...
def convert_pdf_to_structured_data(
    ...
    *,
    ...
    capabilities: Dict[str, Any] = {
        ...
        "ocr": _cap(
            enabled=(text_meta.get("backend") == "rapidocr"),
            disabled_reason=None if text_meta.get("backend") == "rapidocr" else text_meta.get("ocr_error") or "OCR 未命中",
        ),
    ...
    if structured["text"]["meta"].get("backend") != "rapidocr":
        structured["capabilities"]["ocr"] = _cap(
            enabled=False,
            disabled_reason=structured["text"]["meta"].get("ocr_error") or "OCR 未命中",
        )
