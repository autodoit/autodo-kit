...
本模块把 AOB 历史脚本能力统一收敛到 ``autodokit.tools.aob``，
提供两类调用方式：
...
def _run_aob_library_with_repo(argv: list[str], *, repo_root: str = "") -> int:
    """在指定 AOB 仓库根目录下执行 library 工具。

    Args:
        argv: library CLI 参数列表。
        repo_root: AOB 仓库根目录。
    ...
    if str(repo_root).strip():
        os.environ["AOB_REPO_ROOT"] = str(Path(repo_root).expanduser().resolve())
    try:
        return run_aob_library(argv)
    finally:
...
def run_aob_items_sync(*, strategy: str = "mtime_size_then_hash", dry_run: bool = True, repo_root: str = "") -> int:
    ...
    if dry_run:
        args.append("--dry-run")
    return _run_aob_library_with_repo(args, repo_root=repo_root)


def run_aob_aggregate_user_content(
    *,
    ...
    args: list[str] = ["aggregate-user-content"]
    for source_path in list(source_paths or []):
        if str(source_path).strip():
            args.extend(["--source-path", str(source_path).strip()])
    for scope in list(scopes or []):
        if str(scope).strip():
            args.extend(["--scope", str(scope).strip()])
    for project_dir in list(project_dirs or []):
        if str(project_dir).strip():
            args.extend(["--project-dir", str(project_dir).strip()])
    if str(home_dir).strip():
        args.extend(["--home-dir", str(home_dir).strip()])
    if dry_run:
        args.append("--dry-run")
    if skip_items_sync:
        args.append("--skip-items-sync")
    return _run_aob_library_with_repo(args, repo_root=repo_root)


def run_aob_publish_user_content(
    *,
    ...
    args: list[str] = ["publish-user-content"]
    for target_path in list(target_paths or []):
        if str(target_path).strip():
            args.extend(["--target-path", str(target_path).strip()])
    for scope in list(scopes or []):
        if str(scope).strip():
            args.extend(["--scope", str(scope).strip()])
    for project_dir in list(project_dirs or []):
        if str(project_dir).strip():
            args.extend(["--project-dir", str(project_dir).strip()])
    for engine_vendor in list(engine_vendors or []):
        if str(engine_vendor).strip():
            args.extend(["--engine", str(engine_vendor).strip()])
    for ide_vendor in list(ide_vendors or []):
        if str(ide_vendor).strip():
            args.extend(["--ide", str(ide_vendor).strip()])
    if str(home_dir).strip():
        args.extend(["--home-dir", str(home_dir).strip()])
    if include_missing:
...
def run_aob_backup_user_content(
    *,
    target_paths: list[str] | None = None,
    scopes: list[str] | None = None,
    project_dirs: list[str] | None = None,
    ...
    args: list[str] = ["backup-user-content"]
    for target_path in list(target_paths or []):
        if str(target_path).strip():
            args.extend(["--target-path", str(target_path).strip()])
    for scope in list(scopes or []):
    ...
    if str(backup_dir).strip():
        args.extend(["--backup-dir", str(backup_dir).strip()])
    if dry_run:
        args.append("--dry-run")
    return _run_aob_library_with_repo(args, repo_root=repo_root)


def run_aob_update_user_content(
    *,
    target_paths: list[str] | None = None,
    scopes: list[str] | None = None,
    project_dirs: list[str] | None = None,
    ...
    args: list[str] = ["update-user-content"]
    for target_path in list(target_paths or []):
        if str(target_path).strip():
            args.extend(["--target-path", str(target_path).strip()])
    for scope in list(scopes or []):
    ...
    if skip_items_sync:
        args.append("--skip-items-sync")
    if simulate_only:
        args.append("--simulate-only")
    if str(sandbox_dir).strip():
        args.extend(["--sandbox-dir", str(sandbox_dir).strip()])
    return _run_aob_library_with_repo(args, repo_root=repo_root)
...
def run_aob_external_templates_import(
    *,
    ...
    return _run_aob_library_with_repo(import_args, repo_root=str(repo_root_path))
...
def run_aob_workspace_convert(
    *,
    ...
    if str(source_ide).strip():
        args.extend(["--source-ide", str(source_ide).strip()])
    if str(target_ide).strip():
        args.extend(["--target-ide", str(target_ide).strip()])
    if str(source_os).strip():
        args.extend(["--source-os", str(source_os).strip()])
    if str(target_os).strip():
        args.extend(["--target-os", str(target_os).strip()])
    if str(title).strip():
