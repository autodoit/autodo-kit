...
def execute(config_path: Path) -> List[Path]:
    ...
    cfg = ReviewDraftConfig(
        ...
        input_structured_dir=str(merged.get("input_structured_dir")) if merged.get("input_structured_dir") else None,
    ...
    if cfg.input_matrix_jsonl:
    ...
    elif cfg.input_structured_dir or cfg.content_db:
        records = load_document_records_from_structured_source(
            structured_dir=str(cfg.input_structured_dir or ""),
        ...
        for record in records:
            materials.append(
                {
                    "uid": record.get("uid_literature") or record.get("uid"),
                    "title": record.get("title") or record.get("meta", {}).get("title"),
                    "year": record.get("year") or record.get("meta", {}).get("year"),
                    "abstract": record.get("abstract") or record.get("meta", {}).get("abstract"),
                    "text_head": str(record.get("text") or "")[:2000],
                }
            )
        source_note = f"structured: {cfg.input_structured_dir or cfg.content_db}"
    else:
        raise ValueError("请至少提供 input_matrix_jsonl、input_structured_dir 或 content_db")
