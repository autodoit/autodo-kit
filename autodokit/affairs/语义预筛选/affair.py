...
class PrescreenConfig:
    ...
    Attributes:
        content_db: 内容主库路径（由“导入和预处理文献元数据”或项目初始化生成）。
        output_dir: 输出目录。
...
def _resolve_uid_field(df: pd.DataFrame) -> str:
    """解析文献主键字段名。
    ...
    Returns:
        主键字段名，优先 uid_literature，其次 uid。
    """

    return "uid_literature" if "uid_literature" in df.columns else "uid"


def _resolve_has_fulltext_field(df: pd.DataFrame) -> str | None:
    """解析全文可用性字段名。
    ...
    if "has_fulltext" in df.columns:
        return "has_fulltext"
    if "has_pdf" in df.columns:
        return "has_pdf"
    if "是否有原文" in df.columns:
        return "是否有原文"
    return None
...
def _load_table(table_path: Path) -> pd.DataFrame:
    ...
    if not table_path.exists():
        raise FileNotFoundError(f"找不到文献主表：{table_path}")
    df = load_reference_main_table(table_path)
    uid_field = _resolve_uid_field(df)
    df = df.set_index(uid_field, drop=False)
    return df
...
def execute(config_path: Path) -> List[Path]:
    ...
    content_db_path, db_input_key = resolve_content_db_config(
        affair_cfg,
        legacy_keys=("references_db",),
        required=False,
    )
    if content_db_path is None:
        legacy_table = str(affair_cfg.get("input_table_csv") or "").strip()
        if legacy_table:
            candidate = Path(legacy_table)
            if not candidate.is_absolute():
                raise ValueError(
                    "input_table_csv 必须为绝对路径：请确认 main.py 已启用统一路径解析，"
                    f"或检查 workspace_root 配置。当前值={legacy_table!r}"
                )
            content_db_path = candidate
            db_input_key = "input_table_csv"
    ...
    content_db = Path(ps_cfg.content_db)
    if not content_db.is_absolute():
        raise ValueError(
            "content_db 必须为绝对路径：请确认 main.py 已启用统一路径解析，"
            f"或检查 workspace_root 配置。当前值={ps_cfg.content_db!r}"
        )
    ...
    df = _load_table(content_db)
    uid_field = _resolve_uid_field(df)
    has_fulltext_field = _resolve_has_fulltext_field(df)
    if uid_field in df.columns:
        df = df.set_index(uid_field, drop=False)
    ...
    if ps_cfg.include_if_has_pdf and has_fulltext_field:
        has_fulltext_bool = _coerce_bool_series(df[has_fulltext_field])
        base_mask &= has_fulltext_bool.to_numpy(dtype=bool)
    ...
    if include_domains:
        ...
        if exclude_keywords_any:
            ...
            if excl_pattern:
                ...
                for _, r in to_exclude.iterrows():
                    excluded_rows.append({"uid_literature": r.get(uid_field), "title": r.get("title"), "exclude_reason": "命中额外排除词"})
        ...
        kept_uids = set(cand_df[uid_field].astype(str).tolist()) if uid_field in cand_df.columns else set()
        for _, row in df_base.iterrows():
            uid = row.get(uid_field)
            if str(uid) in kept_uids:
                continue
            excluded_rows.append({"uid_literature": uid, "title": row.get("title"), "exclude_reason": "未满足领域交集或命中排除域"})
        ...
        for _, row in filtered_out.iterrows():
            reason = "规则过滤"
            if ps_cfg.include_if_has_pdf and has_fulltext_field:
                has_fulltext = row.get(has_fulltext_field)
                if isinstance(has_fulltext, str):
                    has_fulltext = has_fulltext.strip().lower() in {"1", "true", "yes", "y"}
                if has_fulltext is False:
                    reason = "无本地 PDF"
            if ps_cfg.year_range and row.get("year") is not None:
                reason = "年份不在范围内"
            excluded_rows.append({"uid_literature": row.get(uid_field), "title": row.get("title"), "exclude_reason": reason})
        ...
        for _uid, row in df.iterrows():
            ...
            if ps_cfg.include_if_has_pdf and has_fulltext_field:
                ...
                if isinstance(has_fulltext, str):
                    has_fulltext = has_fulltext.strip().lower() in {"1", "true", "yes", "y"}
                if has_fulltext is False:
                    excluded.append({"uid_literature": uid, "title": row.get("title"), "exclude_reason": "无本地 PDF"})
                    continue
            ...
            if exclude_keywords_any and _contains_any(text, exclude_keywords_any):
                excluded.append({"uid_literature": uid, "title": row.get("title"), "exclude_reason": "命中排除词"})
                continue

            if include_keywords_any and not _contains_any(text, include_keywords_any):
                excluded.append({"uid_literature": uid, "title": row.get("title"), "exclude_reason": "未命中任何包含关键词"})
                continue
            ...
            candidates.append(
                {
                    ...
                    "title": row.get("title"),
                    "year": year,
                    "author": row.get("author") if row.get("author") is not None else row.get("authors"),
