...
def _load_review_read_pool(raw_cfg: Dict[str, Any], workspace_root: Path, content_db: Path) -> pd.DataFrame:
    ...
    def _apply_limit(table: pd.DataFrame) -> pd.DataFrame:
        limited = table.fillna("")
        if max_review_items > 0:
            return limited.head(max_review_items).copy()
        return limited
    ...
    if allow_legacy_queue_fallback:
        ...
        if not queue_df.empty:
            return _apply_limit(queue_df)

    canonical_csv = workspace_root / "views" / "review_candidates" / "review_read_pool.csv"
    if canonical_csv.exists():
        return _apply_limit(pd.read_csv(canonical_csv, dtype=str, keep_default_na=False))
