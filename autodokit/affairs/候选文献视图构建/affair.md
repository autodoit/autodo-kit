...
本事务用于直接从统一内容主库完成综述候选筛选、阅读池构建、综述综合研读与普通文献导种准备。
...
3. 主题筛选参数用于 A050 直题筛选；参考文献处理、笔记骨架、综述综合研读与普通文献导种由当前事务内部 merged phases 一次性完成。
...
8. `gate_review.json`
9. 综述链相关当前态队列与 A080 桥接入口

## 说明

1. 前置筛选阶段允许按研究主题、关键词组和年份窗口直接从内容主库筛出综述候选。
2. 当前事务在一个存活节点内完成综述候选池、阅读池、批次、参考文献处理、综述综合研读和 A080 导种，不再把 A065/A070/A075 暴露为外部节点。
