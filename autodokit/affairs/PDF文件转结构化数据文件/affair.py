...
class PdfDirToStructuredDataConfig:
    ...
    task_type: str = "full_fine_grained"  # 解析任务类型
    content_db: str | None = None  # 可选统一内容主库，用于回写 structured 状态
    db_input_key: str = ""
...
def _build_reference_lookup(content_db: Path) -> tuple[Dict[str, Dict[str, Any]], Dict[str, Dict[str, Any]]]:
    """从 content.db 构建 PDF 路径与文件名到文献记录的映射。"""

    literatures = load_literatures_df(content_db).fillna("")
    attachments = load_attachments_df(content_db).fillna("")
    literature_by_uid = {
        str(row.get("uid_literature") or "").strip(): dict(row)
        for _, row in literatures.iterrows()
    ...
    def register(path_text: str, payload: Dict[str, Any], *, attachment_name: str = "") -> None:
        raw = str(path_text or "").strip()
        if raw:
            path = Path(raw)
            if path.is_absolute():
                by_abs_path[str(path.resolve())] = dict(payload)
            by_name.setdefault(path.name, dict(payload))
        if attachment_name:
            by_name.setdefault(str(attachment_name).strip(), dict(payload))

    for _, row in literatures.iterrows():
        payload = dict(row)
        register(str(row.get("pdf_path") or ""), payload, attachment_name=str(row.get("primary_attachment_name") or ""))

    for _, row in attachments.iterrows():
        uid_literature = str(row.get("uid_literature") or "").strip()
        base = dict(literature_by_uid.get(uid_literature) or {})
        payload = {**base, **dict(row)}
        register(
            str(row.get("storage_path") or row.get("source_path") or ""),
            payload,
            attachment_name=str(row.get("attachment_name") or ""),
        )

    return by_abs_path, by_name
...
def execute(config_path: Path) -> List[Path]:
    ...
    cfg = PdfDirToStructuredDataConfig(
        ...
        task_type=str(affair_cfg.get("task_type") or "full_fine_grained"),
    ...
    if isinstance(cfg.output_structured_dir, str) and cfg.output_structured_dir.strip():
        ...
        workspace_root = infer_workspace_root_from_content_db(cfg.content_db)
        ...
        raise ValueError("output_structured_dir 为空，且未提供 content_db 用于按四组合契约自动推导")
    ...
    reference_lookup_by_path: Dict[str, Dict[str, Any]] = {}
    reference_lookup_by_name: Dict[str, Dict[str, Any]] = {}
    content_db_path_obj: Path | None = None
    if cfg.content_db:
        content_db_path_obj = Path(cfg.content_db)
        if not content_db_path_obj.is_absolute():
            raise ValueError(f"content_db 必须是绝对路径：{content_db_path_obj}")
        reference_lookup_by_path, reference_lookup_by_name = _build_reference_lookup(content_db_path_obj)
    ...
    for i, pdf_path in enumerate(pdf_files, start=1):
        ...
        metadata = reference_lookup_by_path.get(str(pdf_path.resolve())) or reference_lookup_by_name.get(pdf_path.name) or {}
        file_stem = _safe_stem(str(metadata.get("cite_key") or metadata.get("uid_literature") or pdf_path.stem))
        out_json = (output_dir / (file_stem + ".structured.json")).resolve()
        ...
        try:
            if converter == "local_pipeline_v2":
                output_path = convert_pdf_to_structured_data_file_local_v2(
                    pdf_path.resolve(),
                    out_json,
                ...
                output_path = convert_pdf_to_structured_data_file_babeldoc(
                    pdf_path.resolve(),
                    out_json,
            ...
            if content_db_path_obj is not None and str(metadata.get("uid_literature") or "").strip():
                structured_data = load_structured_data(output_path)
                ...
                save_structured_state(
                    content_db_path_obj,
                    uid_literature=str(metadata.get("uid_literature") or "").strip(),
                    structured_status="ready",
                    structured_abs_path=str(output_path),
                    structured_backend=str(metadata.get("structured_backend") or converter),
                    structured_task_type=cfg.task_type,
                    structured_updated_at=str((structured_data.get("parse_profile") or {}).get("created_at") or ""),
                    structured_schema_version=str(structured_data.get("schema") or ""),
    ...
    manifest = {
        ...
        "content_db": str(content_db_path_obj) if content_db_path_obj is not None else "",
        "db_input_key": cfg.db_input_key,
        "converter": converter,
        "overwrite": bool(cfg.overwrite),
        "pdf_count": int(len(pdf_files)),
        "written_structured_count": int(len(written)),
        "registered_structured_count": int(registered_count),
