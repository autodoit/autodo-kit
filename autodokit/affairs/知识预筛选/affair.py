...
from autodokit.tools.storage_backend import load_knowledge_index_table
...
class KnowledgePrescreenEngine:
    ...
    def run(
        ...
        Args:
            focus: 预筛选主题。
            items: 直接传入的知识记录。
            literature_items_path: 旧版文献数据库路径兼容参数。
            content_db: 统一内容主库路径，支持 `.db` 或兼容 CSV。
            knowledge_index_path: 旧版知识索引路径兼容参数。
        ...
        records = items or self._load_items_from_path(content_db or knowledge_index_path or literature_items_path)
        ...
        for record in records:
            text = " ".join(
                [
                    ...
                    str(record.get("aliases") or ""),
                    str(record.get("status") or ""),
                    str(record.get("uid_literature") or ""),
                    str(record.get("cite_key") or ""),
                    " ".join(str(record.get("tags") or "").split("|")),
                    " ".join(str(record.get("evidence_uids") or "").split("|")),
                ]
            )
            ...
            if any(token and token in normalized_text for token in exclude_tokens):
                ...
                excluded.append(
                    KnowledgePrescreenCandidate(
                        item_uid=str(record.get("uid_knowledge") or record.get("item_uid") or ""),
    ...
    def _load_items_from_path(self, knowledge_path: str | Path | None) -> list[dict[str, Any]]:
        """从知识主库或兼容 CSV 加载条目。
        ...
        if knowledge_path is None:
            return []
        raw_path = str(knowledge_path).strip()
        if not raw_path:
            return []
        path = Path(raw_path)
        if not path.exists():
        ...
        if path.suffix.lower() == ".db":
            return load_knowledge_index_table(path).to_dict(orient="records")
...
def execute(config_path: Path) -> List[Path]:
    ...
    result = KnowledgePrescreenEngine().run(
        ...
        content_db=raw_cfg.get("content_db") or raw_cfg.get("knowledge_db"),
        knowledge_index_path=raw_cfg.get("knowledge_index_path"),
