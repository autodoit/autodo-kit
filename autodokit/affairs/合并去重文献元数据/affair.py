...
class DedupConfig:
    ...
    Args:
        input_table_path: 输入文献元数据主表路径（支持 `.db` 或兼容 CSV，应为绝对路径）。
        output_table_path: 输出去重后的路径（支持 `.db` 或兼容 CSV，应为绝对路径）。
...
def dedup_metadata_table(
    *,
    ...
    Args:
        input_table_path: 输入路径（支持 `.db` 或兼容 CSV）。
        output_table_path: 输出路径（支持 `.db` 或兼容 CSV）。
    ...
    in_path = Path(input_table_path)
    out_path = Path(output_table_path)
    ...
    df = load_reference_main_table(in_path)
    if "uid_literature" in df.columns:
        df = df.set_index("uid_literature", drop=False)
    ...
    result = {
        ...
        "input_table_path": str(in_path),
        "output_table_path": str(out_path),
    }
    ...
    persist_reference_main_table(out_df.reset_index(drop=True), out_path)
...
def dedup_metadata_csv(
    *,
    ...
    return dedup_metadata_table(
        input_table_path=input_table_csv,
        output_table_path=output_table_csv,
        dry_run=dry_run,
        backup=backup,
        logger=logger,
    )
...
def _build_cli_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="对文献元数据表做去重并输出新的表")
    p.add_argument("--input-table-path", "--input-table-csv", dest="input_table_path", required=True, help="输入文献表路径（绝对路径，支持 .db 或 CSV）")
    p.add_argument("--output-table-path", "--output-table-csv", dest="output_table_path", required=True, help="输出去重文献表路径（绝对路径，支持 .db 或 CSV）")
...
def execute(config_path: Path, workspace_root: Path | None = None, **_: Any) -> List[Path]:
    ...
    input_table_path = str(cfg.get("input_table_path") or cfg.get("input_table_csv") or "").strip()
    output_table_path = str(cfg.get("output_table_path") or cfg.get("output_table_csv") or "").strip()
    if not input_table_path:
        raise ValueError("配置缺少 input_table_path")
    if not output_table_path:
        raise ValueError("配置缺少 output_table_path")

    dry_run = bool(cfg.get("dry_run", False))
    backup = bool(cfg.get("backup", True))

    dedup_metadata_table(
...
def main(argv: Optional[list] = None) -> int:
    ...
    try:
        res = dedup_metadata_table(
            input_table_path=args.input_table_path,
