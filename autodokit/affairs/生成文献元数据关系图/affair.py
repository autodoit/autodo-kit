...
class Config:
    ...
    Args:
        content_db: 输入的内容主库路径（包含 uid 索引，支持 `.db` 或兼容 CSV）。
        output_dir: 输出目录。
        tag_list: 要匹配的标签列表。
...
def _run_and_write_all_outputs(config_path: Path) -> List[Path]:
    ...
    content_db_path, _ = resolve_content_db_config(merged_cfg, legacy_keys=("references_db",), required=True)
    input_table = Path(str(content_db_path))
    config.content_db = str(input_table)
    output_dir = Path(config.output_dir)

    if not input_table.is_absolute():
        raise ValueError(f"content_db 必须为绝对路径（应由调度层预处理）：{config.content_db!r}")
    ...
    table = load_reference_main_table(input_table)
    if "uid_literature" in table.columns:
        table.set_index("uid_literature", inplace=True, drop=False)
    elif "uid" in table.columns:
        table.set_index("uid", inplace=True, drop=False)
    else:
        raise ValueError("文献主表缺少 uid_literature/uid 列，无法生成关系图")

    if "author" not in table.columns and "authors" in table.columns:
        table["author"] = table["authors"]
